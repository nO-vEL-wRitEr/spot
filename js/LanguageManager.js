class LanguageManager {
  constructor(root = document, spotDB = window.spotDB) {
    ...
    this.translations = {
      en: {
        ...
        'AI': 'AI',
        '프로필': 'Profile',
        '소비 분석': 'Spending Analytics',
        '소비 지도': 'Spending Map',
        '분석 결과': 'Analysis Result',
        '최근 소비': 'Recent Spending',
        '이번 달 총 지출': 'Total Spending This Month',
        'AI 인사이트': 'AI Insights',
        '주간 구매량 추이': 'Weekly Spending Trend',
        '카테고리별 소비': 'Spending by Category',
        '실시간 소비 분석': 'Live Spending Analysis',
        'AI 분석 리포트': 'AI Analysis Report',
        '일간': 'Daily',
        '주간': 'Weekly',
        '월간': 'Monthly',
        '연간': 'Yearly',
        ...
        '구매 내역': 'Items',
        '합계': 'Total',
        '카메라 버튼을 눌러 영수증을 촬영하거나 선택하세요.': 'Tap the camera button to take or choose a receipt photo.',
        '무엇이든 물어보세요': 'Ask anything',
        '입력 즉시 갱신': 'Updates instantly',
        '방문': 'Visits',
        '구매': 'Purchases',
        '총 지출': 'Total Spent',
        '최근 결제': 'Latest Payment',
        '등록된 소비 장소가 없어요.': 'No spending locations yet.',
        '장소 검색': 'Search Places',
        '언어': 'Language'
      },
      ja: {
        ...
        '구매 내역': '購入内容',
        '합계': '合計',
        '카메라 버튼을 눌러 영수증을 촬영하거나 선택하세요.': 'カメラボタンを押してレシートを撮影または選択してください。',
        '무엇이든 물어보세요': '何でも聞いてください',
        '입력 즉시 갱신': '入力後すぐ更新',
        '방문': '訪問',
        '구매': '購入',
        '총 지출': '総支出',
        '최근 결제': '最新の支払い',
        '등록된 소비 장소가 없어요.': '登録された支出場所はありません。',
        '장소 검색': '場所を検索',
        '언어': '言語'
      }
    };
    ...
    for (const lang of ['en', 'ja']) {
      Object.entries(this.translations[lang]).forEach(([ko, value]) => {
        this.reverse[lang][value] = ko;
      });
    }
  }
  ...
  ensureStyles() {
    ...
    style.textContent = `
      .spot-language-wrap{position:absolute;top:12px;right:12px;z-index:70;display:flex;flex-direction:column;align-items:flex-end;gap:7px;pointer-events:none}
      .spot-language-main,.spot-language-option{pointer-events:auto;width:36px;height:36px;border-radius:999px;border:1px solid rgba(8,35,61,.14);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:950;box-shadow:0 5px 16px rgba(8,35,61,.16);cursor:pointer;transition:transform .16s ease,opacity .16s ease,background .16s ease}
      .spot-language-main{background:#08233D;color:#fff;font-size:13px}
      .spot-language-options{display:flex;flex-direction:column;gap:6px;opacity:0;transform:translateY(-7px) scale(.92);pointer-events:none;transition:.16s ease}
      ...
      .spot-language-option:hover,.spot-language-main:hover{transform:scale(1.06)}
    `;
  ...
  createToggle() {
    ...
    if (container.querySelector('#spot-language-toggle')) return;
